...
install_pkgdeps() {
    ...
    sudo apt install -y wget curl git gawk mariadb-common mysql-common policycoreutils \
        python3-matplotlib unixodbc-common firejail python3-pyqt6 nodejs npm \
        python3-pip python3-tk libgtk-3-dev libwebkit2gtk-4.0-dev
}
...
install_selinux() {
    ...
    sudo apt update
    sudo apt install -y selinux-utils selinux-basics policycoreutils policycoreutils-python-utils selinux-policy-default apparmor apparmor-utils lynis || { printf "\033[1;31m[-] Failed to install SELinux and AppArmor dependencies.\033[0m\n"; exit 1; }
    ...
    sudo lynis audit system --quick || printf "\033[1;31m[-] Lynis audit failed. Ensure Lynis is installed and check the logs.\033[0m\n"
...
enable_fail2ban() {
    printf "\033[1;31m[+] Enabling Fail2Ban...\033[0m\n"
    sudo systemctl enable --now fail2ban
    if [ $? -eq 0 ]; then
        printf "\033[1;32m[+] Fail2Ban enabled successfully.\033[0m\n"
    else
        printf "\033[1;31m[-] Failed to enable Fail2Ban. Please check the logs.\033[0m\n"
    fi
}


enable_ufw() {
    printf "\033[1;31m[+] Enabling UFW...\033[0m\n"
    sudo ufw enable
    if [ $? -eq 0 ]; then
        printf "\033[1;32m[+] UFW enabled successfully.\033[0m\n"
    else
        printf "\033[1;31m[-] Failed to enable UFW. Please check the logs.\033[0m\n"
    fi
}
...
install_aide() {
    printf "\033[1;31m[+] Installing AIDE...\033[0m\n"
    if sudo apt list --installed 2>/dev/null | grep -q "^aide/"; then
        printf "\033[1;33m[+] AIDE is already installed. Reinstalling...\033[0m\n"
        if ! sudo apt install --reinstall -y aide; then
            printf "\033[1;31m[-] Failed to reinstall AIDE. Skipping...\033[0m\n" | sudo tee -a /var/log/setup_errors.log > /dev/null
            return 0
        fi
    else
        if ! sudo apt install -y aide; then
            printf "\033[1;31m[-] Failed to install AIDE. Skipping...\033[0m\n" | sudo tee -a /var/log/setup_errors.log > /dev/null
            return 0
        fi
    fi

    if ! sudo aideinit; then
        printf "\033[1;31m[-] Failed to initialize AIDE database. Skipping...\033[0m\n" | sudo tee -a /var/log/setup_errors.log > /dev/null
        return 0
    fi

    if ! sudo mv /var/lib/aide/aide.db.new /var/lib/aide/aide.db; then
        printf "\033[1;31m[-] Failed to move AIDE database. Skipping...\033[0m\n" | sudo tee -a /var/log/setup_errors.log > /dev/null
        return 0
    fi

    printf "\033[1;31m[+] AIDE database initialized.\033[0m\n"

    if ! sudo aide --check; then
        printf "\033[1;31m[-] AIDE check failed. Skipping...\033[0m\n" | sudo tee -a /var/log/setup_errors.log > /dev/null
        return 0
    fi

    printf "\033[1;31m[+] AIDE check completed.\033[0m\n"
}
...
stig_login_banners() {
    echo "You are accessing a fully secured [SIG] Security International Group Information System (IS)..." > /etc/issue
...
stig_secure_filesystem() {
    ...
    if ! getent passwd > /dev/null || ! getent shadow > /dev/null; then
        printf "\033[1;31m[-] Error: Permissions may have broken user logins. Reverting changes...\033[0m\n"
...
-a always,exclude -F msgtype=USER_AUTH -F auid>=1000 -F auid!=4294967295 -k exclude_user_auth
-e 2
EOF
...
stig_disable_usb() {
    ...
    echo "blacklist usb-storage" >> /etc/modprobe.d/hardn-blacklist.conf
    echo "blacklist uas" >> /etc/modprobe.d/hardn-blacklist.conf
    sudo update-initramfs -u || printf "\033[1;31m[-] Failed to update initramfs.\033[0m\n"
    lsmod | grep usb_storage || printf "\033[1;32m[+] USB storage successfully disabled while allowing HID devices to work.\033[0m\n"
}
...
stig_disable_ipv6() {
    echo "net.ipv6.conf.all.disable_ipv6 = 1" | sudo tee -a /etc/sysctl.d/99-sysctl.conf > /dev/null
    echo "net.ipv6.conf.default.disable_ipv6 = 1" | sudo tee -a /etc/sysctl.d/99-sysctl.conf > /dev/null
    echo "net.ipv6.conf.default.disable_ipv6 = 1" | sudo tee -a /etc/sysctl.d/99-sysctl.conf > /dev/null
    sudo sysctl -p
}
...
stig_configure_firewall() {
    ...
    sudo mkdir -p /etc/iptables && sudo iptables-save > /etc/iptables/rules.v4 || printf "\033[1;31m[-] Warning: Could not save iptables rules.\033[0m\n"
    printf "\033[1;31m[+] Reloading iptables rules...\033[0m\n"
    sudo iptables-restore < /etc/iptables/rules.v4 || printf "\033[1;31m[-] Warning: Could not reload iptables rules.\033[0m\n"
    printf "\033[1;31m[+] Verifying iptables rules...\033[0m\n"
    sudo iptables -L -v || printf "\033[1;31m[-] Warning: Could not verify iptables rules.\033[0m\n"
    printf "\033[1;31m[+] iptables configuration completed.\033[0m\n"

    printf "\033[1;31m[+] Reloading network connections...\033[0m\n"
    sudo systemctl restart networking || printf "\033[1;31m[-] Warning: Could not restart networking service.\033[0m\n"

    sudo sed -i 's/^PermitRootLogin.*/PermitRootLogin no/' /etc/ssh/sshd_config
    sudo sed -i 's/^#Ciphers.*/Ciphers aes256-ctr,aes192-ctr,aes128-ctr/' /etc/ssh/sshd_config
    sudo sed -i 's/^#MACs.*/MACs hmac-sha2-512,hmac-sha2-256/' /etc/ssh/sshd_config
    sudo sed -i 's/^#RekeyLimit.*/RekeyLimit 1G 1h/' /etc/ssh/sshd_config
    sudo systemctl restart sshd
}


stig_enforce_network() {
    ...
    sudo yum install -y yum-plugin-verify
    sudo yum verify all

    printf "\033[1;31m[+] Restricting firewall changes to root...\033[0m\n"
    sudo chmod 700 /sbin/iptables
    sudo chmod 700 /sbin/iptables-save
    sudo chmod 700 /sbin/iptables-restore
}
...
apply_stig_hardening() {
    ...
    if ! stig_password_policy; then
        printf "\033[1;31m[-] Failed to apply password policy.\033[0m\n"
        return 1
    fi

    if ! stig_lock_inactive_accounts; then
        printf "\033[1;31m[-] Failed to lock inactive accounts.\033[0m\n"
        return 1
    fi

    if ! stig_login_banners; then
        printf "\033[1;31m[-] Failed to set login banners.\033[0m\n"
        return 1
    fi

    if ! stig_secure_filesystem; then
        printf "\033[1;31m[-] Failed to secure filesystem permissions.\033[0m\n"
        return 1
    fi

    if ! stig_kernel_setup; then
        printf "\033[1;31m[-] Failed to configure kernel parameters.\033[0m\n"
        return 1
    fi

    if ! stig_audit_rules; then
        printf "\033[1;31m[-] Failed to configure audit rules.\033[0m\n"
        return 1
    fi

    if ! stig_disable_usb; then
        printf "\033[1;31m[-] Failed to disable USB storage.\033[0m\n"
        return 1
    fi

    if ! stig_disable_core_dumps; then
        printf "\033[1;31m[-] Failed to disable core dumps.\033[0m\n"
        return 1
    fi

    if ! stig_disable_ctrl_alt_del; then
        printf "\033[1;31m[-] Failed to disable Ctrl+Alt+Del.\033[0m\n"
        return 1
    fi

    if ! stig_disable_ipv6; then
        printf "\033[1;31m[-] Failed to disable IPv6.\033[0m\n"
        return 1
    fi

    if ! stig_enforce_network; then
        printf "\033[1;31m[-] Failed to enforce network settings.\033[0m\n"
        return 1
    fi

    if ! stig_configure_firewall; then
        printf "\033[1;31m[-] Failed to configure firewall.\033[0m\n"
        return 1
    fi

    if ! stig_enforce_apparmor; then
        printf "\033[1;31m[-] Failed to enforce AppArmor.\033[0m\n"
        return 1
    fi

    if ! stig_set_randomize_va_space; then
        printf "\033[1;31m[-] Failed to set randomize_va_space.\033[0m\n"
        return 1
    fi

    if ! update_firmware; then
        printf "\033[1;31m[-] Failed to update firmware.\033[0m\n"
        return 1
    fi
...
PACKAGES_SCRIPT="$(dirname "$0")/packages.sh"
CURRENT_USER=$(whoami)
...
if [ -f "$PACKAGES_SCRIPT" ]; then
    ...
    if chmod +x "$PACKAGES_SCRIPT"; then
        printf "\033[1;32m[+] Executable permissions set successfully.\033[0m\n"
    else
        printf "\033[1;31m[-] Failed to set executable permissions for packages.sh.\033[0m\n"
        exit 1
    fi


    printf "\033[1;31m[+] Setting sudo permissions for packages.sh...\033[0m\n"
    if echo "$CURRENT_USER ALL=(ALL) NOPASSWD: $PACKAGES_SCRIPT" | sudo tee /etc/sudoers.d/packages-sh > /dev/null; then
        printf "\033[1;32m[+] Sudo permissions set successfully.\033[0m\n"
    else
        printf "\033[1;31m[-] Failed to set sudo permissions for packages.sh.\033[0m\n"
        exit 1
    fi
    ...
    if sudo "$PACKAGES_SCRIPT"; then
        printf "\033[1;32m[+] packages.sh executed successfully.\033[0m\n"
    else
        printf "\033[1;31m[-] Failed to execute packages.sh. Check the script for errors.\033[0m\n"
        exit 1
    fi
else
...
main() {
    ...
    printf "\033[1;31m========================================================\033[0m\n"
    printf "\033[1;31m             [+] HARDN - Installing AIDE                \033[0m\n"
    printf "\033[1;31m       [+] AIDE will not run until after reboot         \033[0m\n"
    printf "\033[1;31m========================================================\033[0m\n"
    install_aide
