...
# Disabe unicode:
# Ensures consistant behaviour accorss systems
# Prevents locale-Based vulns
# Improves perfomance, and more
LC_ALL=C
LANG=C

# Alternative to sleep that uses read with a timeout
# sleep is an external command and not a bash built-in
read_sleep() {
    # Usage: read_sleep 1
    #        read_sleep 0.2
    read -rt "$1" <> <(:) || :
...
setup_security(){
        ...
        # Load each module in the array. C Style loop for increased perfomance.
        ...
        #
        # Possible solution: Use the FTP server to download the latest version of chkrootkit.
        # Attempting to use FTP
        # src/setup/modules/chkrootkit.sh
        #
